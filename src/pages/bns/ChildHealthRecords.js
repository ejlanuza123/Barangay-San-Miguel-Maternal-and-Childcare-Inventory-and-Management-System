...
// NEW: Fully functional Pagination component
const Pagination = ({ currentPage, totalPages, onPageChange }) => {
    return (
        <nav className="flex items-center justify-center space-x-1 text-xs mt-4">
            <button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1} className="px-2 py-1 rounded hover:bg-gray-200 disabled:opacity-50">&lt;</button>
            <span className="px-2 py-1 font-semibold">Page {currentPage} of {totalPages}</span>
            <button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages} className="px-2 py-1 rounded hover:bg-gray-200 disabled:opacity-50">&gt;</button>
        </nav>
    );
};

// NEW: View modal for child records
const ViewChildModal = ({ child, onClose }) => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
        ...
        >
            <div className="p-6">
                <h2 className="text-xl font-bold text-gray-800 mb-4">View Child Record</h2>
                <div className="space-y-2 text-sm">
                    <p><span className="font-semibold">Child ID:</span> {child.child_id}</p>
                    <p><span className="font-semibold">Name:</span> {child.first_name} {child.last_name}</p>
                    <p><span className="font-semibold">Date of Birth:</span> {child.dob}</p>
                    <p><span className="font-semibold">Mother's Name:</span> {child.mother_name}</p>
                    <p><span className="font-semibold">Guardian's Name:</span> {child.guardian_name}</p>
                    <p><span className="font-semibold">Nutrition Status:</span> <StatusBadge status={child.nutrition_status} /></p>
                </div>
...
export default function ChildHealthRecords() {
    ...
    const fetchPageData = useCallback(async () => {
        ...
        const { data: appointmentsData, error: appointmentsError } = await supabase.from('appointments').select('*').order('date', { ascending: true }).limit(3);
    ...
    useEffect(() => {
        fetchPageData();
    }, [fetchPageData, currentPage]); // Re-fetch when currentPage changes
                ...
                <div className="xl:col-span-3 bg-white p-4 rounded-lg shadow-sm border">
                    <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-3">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <button className="flex items-center space-x-2 px-3 py-1.5 text-sm border rounded-md bg-white hover:bg-gray-50"><FilterIcon /> <span>Filter</span></button>
                        </div>
                        <button onClick={() => setModalMode('add')} className="w-full sm:w-auto bg-blue-600 text-white font-bold py-2 px-4 rounded-lg shadow-md hover:bg-blue-700 text-sm whitespace-nowrap">+ Add New Patient</button>
                    </div>
