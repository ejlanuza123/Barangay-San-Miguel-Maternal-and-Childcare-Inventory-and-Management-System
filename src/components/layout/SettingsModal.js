...
const BellIcon = () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6 6 0 10-12 0v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>;
...
const ToggleSwitch = ({ label, description, isEnabled, onToggle }) => (
    <div className="flex justify-between items-center py-2">
        ...
        <button onClick={onToggle} className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors ${isEnabled ? 'bg-blue-600' : 'bg-gray-300'}`}>
    ...
    return (
        <div>
            ...
            <form onSubmit={handleUpdate} className="space-y-6">
                ...
                <div className="pt-4"><button type="submit" className="bg-blue-600 text-white font-semibold py-2.5 px-8 rounded-lg shadow-sm hover:bg-blue-700">Update</button></div>
            </form>
        </div>
    );
};
...
const NotificationSettings = ({ initialPrefs, onUpdate }) => {
    ...
    return (
        <div>
            ...
            <p className="text-gray-500 mb-8">Manage how you receive important reminders and updates about appointments, inventory, and announcements. Choose your preferred channels and customize your notification experience.</p>
            <div className="space-y-8">
                <div>
                    <h3 className="text-lg font-bold text-gray-800 mb-3">Notification Types</h3>
                    <div className="space-y-3">
                        <ToggleSwitch label="Appointment Reminders" description="Alerts for upcoming prenatal check-ups, postnatal visits, and child immunizations." isEnabled={prefs.appointment_reminders} onToggle={() => handleToggle('appointment_reminders')} />
                        <ToggleSwitch label="Inventory Alerts" description="Notifications about low stock or expiring medical supplies." isEnabled={prefs.inventory_alerts} onToggle={() => handleToggle('inventory_alerts')} />
                        <ToggleSwitch label="Patient Follow-ups" description="Reminders for overdue check-ups, missed appointments, or follow-up actions." isEnabled={prefs.patient_followups} onToggle={() => handleToggle('patient_followups')} />
                    </div>
                </div>
                <div>
                    <h3 className="text-lg font-bold text-gray-800 mb-3">Notification Channels</h3>
                     <div className="space-y-3">
                        <ToggleSwitch label="In-app Notifications" description="Real-time alerts within the web and mobile apps." isEnabled={prefs.in_app_notifications} onToggle={() => handleToggle('in_app_notifications')} />
                        <ToggleSwitch label="SMS Notifications" description="Text message reminders for users with limited internet access." isEnabled={prefs.sms_notifications} onToggle={() => handleToggle('sms_notifications')} />
                        <ToggleSwitch label="Email Notifications" description="Optional email updates for those who prefer email." isEnabled={prefs.email_notifications} onToggle={() => handleToggle('email_notifications')} />
                    </div>
                </div>
            </div>
        </div>
...
export default function SettingsModal({ onClose }) {
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'My Profile': return <MyProfile profile={profile} onProfileUpdate={handleProfileUpdate} />;
            case 'Notification Setting': return <NotificationSettings initialPrefs={profile?.preferences} onUpdate={handleUpdatePreferences} />;
            case 'About': return <AboutSection />;
            default: return <div className="text-center p-8 text-gray-500">{activeTab} section coming soon.</div>;
        }
    };
                ...
                <div className="flex flex-col space-y-1">
                    {navItems.map(item => (
                        <button key={item.name} onClick={() => setActiveTab(item.name)}
                            className={`flex items-center space-x-3 p-3 rounded-lg text-left font-semibold text-base transition-colors w-full ${activeTab === item.name ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-200'}`}>
                            {item.icon}
                            <span>{item.name}</span>
                        </button>
                    ))}
